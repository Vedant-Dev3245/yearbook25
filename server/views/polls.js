...
const allPolls = async (req, res) => {
    const questions = await Poll.find({}).select(["ques", "totalCount", "votes"]).populate({ path: 'votes', populate: { path: 'user' }});
...
const getPoll = async (req, res) => {
    try {
        ...
        const poll = await Poll.findById(pollID).populate({ path: 'votes', populate: { path: 'user' }});
...
const votePoll = async (req, res) => {
    try {
        ...
        if (voterUser.hasVoted != false) return res.status(400).json({ msg: "You have already voted" });
